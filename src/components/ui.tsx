  ...
  const drawn = segments
    ...
    .map((segment, index) => {
      ...
      return {
        ...
        start,
        length,
        // Соседние сегменты различаются прозрачностью одного цвета, а не
        // разными цветами: радуга из пяти оттенков в кольце читалась бы
        // как шкала, которой тут нет. Границу держит в основном зазор,
        // прозрачность только помогает — иначе бледный сегмент выглядел бы
        // как незаполненная часть нормы
        opacity: [1, 0.68, 0.86, 0.56][index % 4],
      };
    });
