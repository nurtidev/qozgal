...
describe('Карточка разбора', () => {
  ...
  test('расчётная карточка помечена как приблизительная', () => {
    // Непроверенными бывают и карточки местной кухни (расчёт по рецептуре),
    // и усреднённые блюда из справочника USDA — в обоих случаях число
    ...
    assert.match(text, /калорийность оценочная, а не измеренная/);
  });
